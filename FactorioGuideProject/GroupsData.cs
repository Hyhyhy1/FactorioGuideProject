    ...
    public class GroupsData
	{
        ...
        public static Label[] GetTipsTexts()
        {
            ...
            group.Add(new Label()
            {
                ...
                Environment.NewLine + "Не бойтесь логических сетей, с их помощью можно делать множество интересных вещей. Они также полезны для оптимизации производства.",
                Dock = DockStyle.Fill,
                ForeColor = Color.White,
            });
        ...
        public static Label GetFinalText()
        {
            var text = new Label()
            {
                Text = "Ура! Мы запустили ракету. На всю игру было потрачено 80 часов. Мы избегали стычек с кусаками, следили за загрязнением. " +
                "Нам пришлось развиваться и находить новые месторождения, потому что была нехватка первоначальных ресурсов. " +
                "Ушло много времени на то, чтобы разобраться с тем, как вообще работает мир Factorio и что нужно делать. " +
                "Но после того, как мы разобрались с автоматизацией, мы выработали общий план: \"Добывай-исследуй-автоматизируй\".Мы желаем вам успешного прохождения и надеемся, что игра принесет вам только удовольствие и вы сможете пройти ее быстрее :)",
        ...
        public static Label GetFinalLabel()
        {
            return new Label() { Text = "Финал", Dock = DockStyle.Fill, ForeColor = Color.White, };
        }
    }
}
